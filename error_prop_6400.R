# Analysis of LI-COR 6400 internal calculation error
# Equations refer to LI-COR 6400 operators manual
# 10 Dec 2019
# JCG
...
#input_data = read.csv("error_test_data_6400.csv")


est_error_6400_max <- function(test_params, T_leaf_error = 1.25) {
  ...
  for (i in 1:n){
    ...
    results = rbind(results, data.frame(g_sw_0, g_sw_a, g_sw_b,
                    g_tw_0,g_tw_a,g_tw_b,
                    g_tc_0,g_tc_a,g_tc_b,
                    C_i_0,C_i_a,C_i_b))
  }
...
# This function will use a regression of error in Tleaf against the difference between
# Tleaf and Tair to get an estimate of Tleaf error (and derived qtys) based on
# actual conditions. I suspect this

est_error_6400_reg <- function(test_params, m = 1, b = 0) {
...
#  T_error = T_leaf_error  # Estimate of upper bound error in T_leaf
  ...
  for (i in 1:n){
    ...
    E_cur = test_params[i,]$Trans       # Transpiration rate
    TleafC_cur = test_params[i,]$Tleaf  # Leaf Temperature C
    TairC_cur = test_params[i,]$Tair    # Air Temperature C
    P_cur = test_params[i,]$Press       # Air pressure
    W_s_cur = test_params[i,]$H2OS      # H20 in sample
    g_bw_cur = test_params[i,]$BLC_1    # Boundary layer conductance
    K_cur = test_params[i,]$StmRat      # Stomatal ratio
...
#    C_i_b = C_i(g_tc = g_tc_b, C_s = C_s_cur, A = A_cur, E = E_cur)
  ...
  # Combine positive and negative errors (now not - add back in abs)
