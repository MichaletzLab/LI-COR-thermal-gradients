...
make_fig5 = function() {
  ...
  # Open file
  pdf("figures/fig5.pdf", width = 3, height = 2.65)
  ...
  p1 = ggplot(data=Tleaf_error_trials_all, aes(x=T_air-T_leaf, y=T_leaf-T_below, color=Licor_Type)) +
    ...
    ylim(c(-3.1,4.4)) +
    xlab(expression(paste(T[air], " - ", T[leaf]," (", degree, "C)"))) + 
    ylab(expression(paste(T[leaf], " error (", degree, "C)"))) #+
    #annotate("text", x = -3.3, y = 4.4, label = "(b)")
  # 
