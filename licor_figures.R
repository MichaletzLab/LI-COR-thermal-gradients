...
pdf("fig1.pdf", width = 7.5, height = 3.5)
...
pdf("fig2.pdf", width = 7.5, height = 7)
...
p1 = ggplot(data = fig2a_1, aes(x=T_air,y=T_air_meas,color=above_below)) + 
  ...
  scale_shape_manual(values=c(21,22,23), labels = c("Empty cuvette", expression(italic("G. shallon")), expression(italic("T. plicata")))) +
  geom_abline(slope = 1, linetype = "dashed") +
  geom_smooth(method=lm) +
  my_theme +
  guides(color = guide_legend(override.aes = list(pch = 21))) +
...
pdf("fig4.pdf", width = 7, height = 3.5)
...
pdf("fig5.pdf", width = 7, height = 3.5)
...
# z = lm(T_below ~ T_air_avg*Licor_Type, data = Tleaf_error_trials_all)
# anova(z)
# summary(z)
